...
時系列データについて、もともとのデータをある単位時間で集約したい場合は`resample()`を使う。引数の`freq`で集約したい単位を指定するが、上記では`'M'`を指定して、月単位での集計を行っている。これを`'Q'`とすると、四半期ごとの集計となる。[ここ](https://note.nkmk.me/python-pandas-time-series-freq/)に指定方法が書いてある。
...
平均を求める`mean()`、最大値を求める`max()`、最小値を求める`min()`、標準偏差を求める`std()`、分散を求める`var()`をそれぞれ実行してみた。注意点は、上記の場合あくまでも`isnull()`の結果、つまり真偽の結果のみを対象として各統計量を求めているため、最大値はTrueの1になるし最小値はFalseの0になる。リサンプルした結果、単位時間あたりで欠損値が1件も存在しなければ標準偏差や分散は0になる。よって、`isnull()`の結果に対してこれらの統計量を算出するのはあまり意味がないかもしれない。

ちなみに、Trueを1としてカウントするのを応用すれば、欠損値以外の条件でもカウントができる。たとえば、数値データが対象ならある一定の大きさであるデータが1月あたり何件存在するかといった集計も可能。
...
条件を`&`で連結すればANDになるし、`|`で連結すればORになる。出力結果はデータフレームになるので、そのままチェーンメソッドを使って`resample()`を指定し`sum()`を実行すれば「条件に合致するデータ数」がカウントできる。
